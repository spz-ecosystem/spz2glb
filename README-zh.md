...
- **SPZ 头部预检**: 转换前 JS 侧 `detectSpzVersion` 读取魔数（v3 `1F8B` / v4 `NGSP`）识别，非法文件提前报错，不进入 WASM 转换；面板同时显示最近一次转换的 SPZ 版本（v3 gzip / v4 zstd）。
- **分段计时**: 性能面板区分 WASM 转换耗时与 JS 侧开销，定位耗时到底花在哪。
- **双语切换 + 日夜主题**: 顶部工具栏中/EN 语言切换与白天/黑夜主题切换，均持久化到 localStorage。
- **冷启动提示**: 面板提示首次转换含冷启动（v3 gzip 首次 zlib 解压初始化），之后会更快。
- **构建/加载时间戳 + 缓存防呆**: 面板显示 CI 构建时间与 WASM 加载时间戳，确认没有命中旧缓存二进制。
- **自动下载 + 队列自动清理**: 转换完成自动下载 GLB 与 JSON 报告，稍后自动清理队列；完成项滑出动画。
- **JSON 报告导出**: 每个完成的转换可下载 JSON 报告，包含 SPZ/GLB/KHR 完整元数据和时间戳。
- **CLI 兼容**: 导出的 JSON 报告可通过 `spz_verify --report <file.json>` 在 CLI 端验证。
- **运行时性能面板**: 自动展示运行时统计（WASM 版本、设备信息、内存统计、分配/释放/失败计数、热池、工作区使用、推荐文件大小上限、SPZ 版本）。
